...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Kaspacore methods. For example, calling `JSON.stringify` on an Kaspacore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'kaspacore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the kaspacore object.
...
```sh
git remote add bitpay git@github.com:aspectron/kaspacore.git
...
Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of kaspacore.
