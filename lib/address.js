 ...
 * `isPayToPublicKey` and `isPayToScriptHash`
 *
 * @example
 * ```javascript
...
Address._transformPublicKey = function(pubkey) {
  ...
  info.hashBuffer = pubkey.toBuffer();
...
function decodeCashAddress(address) {
  ...
  var hashOrPublicKey = convertedBits;

  if (versionByte == 1){
    $.checkArgument(264 === hashOrPublicKey.length * 8, 'Invalid hash size:'+ address);
  }else{
    $.checkArgument(256 === hashOrPublicKey.length * 8, 'Invalid hash size:'+ address);
  }
...
//console.log('[address.js.339]', hash); //TODO
  ...
  if (type == 'pubkey'){
    info.hashBuffer = Buffer.from(hashOrPublicKey);
  }else{
    info.hashBuffer = Buffer.from(hashOrPublicKey);
  }
 ...
 * @return boolean
 */
Address.prototype.isPayToPublicKey = function() {
