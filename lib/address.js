 ...
 * `Address.PayToPublicKey` (value is the `'pubkey'` string)
...
function Address(data, network, type) {
  ...
  if (type && (type !== Address.PayToPublicKey && type !== Address.PayToScriptHash)) {
    throw new TypeError('Third argument must be "pubkey" or "scripthash".');
  }

  var info = this._classifyArguments(data, network, type);

  //if(!info.publicKeyBuffer){
    //console.log("infoinfo", info, data, network, type)
    //throw new Error("DDDDDDD")
  //}

  // set defaults if not set
  info.network = info.network || Networks.get(network) || Networks.defaultNetwork;
  info.type = info.type || type || Address.PayToPublicKey;

  JSUtil.defineImmutable(this, {
    publicKeyBuffer: info.publicKeyBuffer,
...
Address.prototype._classifyArguments = function(data, network, type) {
  ...
  if ((data instanceof Buffer || data instanceof Uint8Array) && data.length === 32) {
    return Address._transformPublicKeyBuffer(data);
 ...
 * @param {string} data.type - either 'pubkey' or 'scripthash'
...
Address._classifyFromVersion = function(buffer) {
  ...
  //console.log("xxxxxxx::::", buffer[0], pubkeyNetwork)
  
  if (pubkeyNetwork) {
    version.network = pubkeyNetwork;
    version.type = Address.PayToPublicKey;
 ...
 * @param {string=} type - The type: 'pubkey' or 'scripthash'
...
Address._transformBuffer = function(buffer, network, type) {
  ...
  if(info.type == "pubkey"){
    info.publicKeyBuffer = buffer.slice(1);
  }else{
...
Address._transformPublicKey = function(pubkey) {
  ...
  info.publicKeyBuffer = pubkey.toBuffer();
  info.type = Address.PayToPublicKey;
  return info;
};

/**
 * Internal function to transform a {@link Buffer}
 *
 * @param {Buffer} pubkeyBuffer - A buffer of the public key
 * @returns {Object} An object with keys: hashBuffer, type
 * @private
 */
 Address._transformPublicKeyBuffer = function(pubkeyBuffer) {
  var info = {};
  if (!(pubkeyBuffer instanceof Buffer)) {
    throw new TypeError('pubkeyBuffer must be a buffer');
  }
  info.publicKeyBuffer = pubkeyBuffer;
  info.type = Address.PayToPublicKey;
  return info;
};
...
function decodeCashAddress(address) {
  ...
  let sliced = payload.slice(0, -8)
  //console.log("sliced:", sliced)
  var convertedBits = convertBits(sliced, 5, 8, true);
  //console.log("convertedBits:", convertedBits)
  var versionByte = convertedBits.shift();
  var hash = convertedBits;

  $.checkArgument(getHashSize(versionByte) === hash.length * 8, 'Invalid hash size:'+ address+`\n versionByte:${versionByte} length: ${getHashSize(versionByte)} !== ${hash.length * 8}`);
  ...
  if(type == "pubkey"){
    info.publicKeyBuffer = Buffer.from(hash);
  }else{
...
Address._transformString = function(data, network, type) {
  //console.log("_transformString:", data, network, type)
...
Address.fromPublicKey = function(data, network) {
  ...
  return new Address(info.publicKeyBuffer, network, info.type);
};

/**
 * Instantiate an address from a public key
 *
 * @param {Buffer} data - A buffer of the public key
 * @param {String|Network} network - either a Network instance, 'livenet', or 'testnet'
 * @returns {Address} A new valid and frozen instance of an Address
 */
 Address.fromPublicKeyBuffer = function(data, network) {
  var info = Address._transformPublicKeyBuffer(data);
  //console.log("fromPublicKeyBuffer:data, network", data, network, info)
  return new Address(info.publicKeyBuffer, network, Address.PayToPublicKey);
};
...
Address.prototype.isPayToPublicKeyHash = function() {
  return this.type === Address.PayToPublicKey;
};
...
Address.prototype.toCashAddress = function(stripPrefix) {
  ...
  var versionByte = getTypeBits(this.type)
  //console.log("this.publicKeyBuffer", this.type, this.publicKeyBuffer, "versionByte:"+versionByte)
  var arr =  Array.prototype.slice.call(this.publicKeyBuffer, 0);
