...
Schnorr.prototype._findSignature = function(d, e) {
  ...
  Schnorr.sign = function(hashbuf, privkey, endian) {
    ...
    */
    ...
    let result = secp256k1.schnorrsig_sign(privkey.toString(), hashbuf);
    
    let sig = Signature.fromString(result.sig);
    let b = sig.toBuffer("schnorr").toString("hex");
    if(b.length < 128)
      throw new Error(`Invalid Signature\nsecp256k1 sig:${result.sig}\nSignature.fromString:${b}`)
    //console.log("schnorrsig_sign sig.length:", result.sig.length, b.length, b)
    sig.compressed = true;
    return sig;
  }
  ...
  /*
  let str = "B341907592EE417F7433FDBE357B44B6837E2425FBE5CC1DE201585F07CB892B00DBE36182F03F0784FAA4D00F0384749E7002CE56DFF1FDED79A14C47399624";
  let buf = Buffer.from(str, "hex");
  let obj = Signature.parseSchnorrEncodedSig(buf);
  let _sig = new Signature();
  _sig.r = obj.r;
  _sig.s = obj.s;
  _sig.isSchnorr = true;
  let sig = Signature.fromString(str);
  let s = sig.toBuffer("schnorr").toString("hex");
  let s2 = _sig.toBuffer("schnorr").toString("hex");
  let cc = _sig.s.toBuffer({size1:32})
  console.log("s:", _sig.s, cc.toString("hex").toUpperCase(), cc.length)
  //console.log("sig:", s.length, buf.length, buf, obj, _sig, s2, s)
  */
