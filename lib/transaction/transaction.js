...
Transaction.MassPerSigOp = 10000;
...
Transaction.prototype.getStandaloneMass = function() {
	...
	return txSize * Transaction.MassPerTxByte + scriptPubKeySize * Transaction.MassPerScriptPubKeyByte
}
Transaction.prototype.getMass = function() {
	let standaloneMass = this.getStandaloneMass();
	let sigOpsCount = 0;
	this.inputs.forEach(input=>{
		//console.log("input.script", input.output.script)
	...
	return standaloneMass + sigOpsCount * Transaction.MassPerSigOp;
}
...
Transaction.prototype.sign = function(privateKey, sigtype, signingMethod) {
	...
	this.inputs.map((input, index)=>{
		let address = input.output.script.getPublicKeyHash().toString("hex");
...
Transaction.prototype.getSignatures = function(privKey, sigtype, signingMethod) {
	...
	let hashData = Hash.sha256ripemd160(privKey.publicKey.toBuffer().slice(1));
	//if(1){
